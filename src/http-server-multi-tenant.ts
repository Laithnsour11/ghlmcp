/**
 * GoHighLevel MCP HTTP Server with Multi-Tenant Support
 * HTTP version for ChatGPT web integration with multi-tenancy
 */
...
import { createTenantStore } from './storage/tenant-store.js';
import { TenantResolver } from './core/tenant-resolver.js';
import { TenantConfigManager } from './core/tenant-config-manager.js';
import { RequestContext, runWithContext, getCurrentContext } from './core/request-context.js';
import { 
...
/**
 * HTTP MCP Server class with multi-tenant support
 */
class GHLMCPHttpMultiTenantServer {
  private app: express.Application;
  private server: Server;
  
  // Multi-tenant components
  private tenantStore = createTenantStore();
  private tenantResolver = new TenantResolver(this.tenantStore);
  private tenantConfigManager = new TenantConfigManager(this.tenantStore);
  
  // Legacy single-tenant support
  private singleTenantClient?: GHLApiClient;
  
  // Tool instances (now support multi-tenancy)
  ...
  constructor() {
    ...
    // Initialize tools (now with multi-tenant support)
  ...
  private setupMiddleware(): void {
    // Tenant resolution middleware (for most routes)
    this.app.use(createTenantMiddleware({
      tenantStore: this.tenantStore,
      tenantResolver: this.tenantResolver,
      tenantConfigManager: this.tenantConfigManager,
      requireTenant: false, // Don't require for all routes
      excludePaths: ['/health', '/', '/admin', '/tenants'] // Exclude admin and health routes
    }));

    // Rate limiting per tenant
    this.app.use(createTenantRateLimitMiddleware({
  ...
  private initializeTools(): void {
    // Initialize legacy single-tenant client if env vars present
    if (process.env.GHL_API_KEY && process.env.GHL_LOCATION_ID) {
      this.singleTenantClient = this.initializeLegacyClient();
    }
    
    // Initialize tools - they now support both single and multi-tenant modes
    this.contactTools = new ContactTools(this.singleTenantClient);
    this.conversationTools = new ConversationTools(this.singleTenantClient);
    this.blogTools = new BlogTools(this.singleTenantClient);
    this.opportunityTools = new OpportunityTools(this.singleTenantClient);
    this.calendarTools = new CalendarTools(this.singleTenantClient);
    this.emailTools = new EmailTools(this.singleTenantClient);
    this.locationTools = new LocationTools(this.singleTenantClient);
    this.emailISVTools = new EmailISVTools(this.singleTenantClient);
    this.socialMediaTools = new SocialMediaTools(this.singleTenantClient);
    this.mediaTools = new MediaTools(this.singleTenantClient);
    this.objectTools = new ObjectTools(this.singleTenantClient);
    this.associationTools = new AssociationTools(this.singleTenantClient);
    this.customFieldV2Tools = new CustomFieldV2Tools(this.singleTenantClient);
    this.workflowTools = new WorkflowTools(this.singleTenantClient);
    this.surveyTools = new SurveyTools(this.singleTenantClient);
    this.storeTools = new StoreTools(this.singleTenantClient);
    this.productsTools = new ProductsTools(this.singleTenantClient);
    this.paymentsTools = new PaymentsTools(this.singleTenantClient);
    this.invoicesTools = new InvoicesTools(this.singleTenantClient);
  }

  /**
   * Initialize legacy single-tenant GHL API client
   */
  private initializeLegacyClient(): GHLApiClient {
    ...
    console.log('[GHL MCP HTTP] Initialized legacy single-tenant client');
  ...
  private setupMCPHandlers(): void {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        if (context) {
          context.log(`Tool ${name} executed successfully`);
        } else {
        ...
        if (context) {
          context.error(`Error executing tool ${name}`, error);
        } else {
  ...
  private setupRoutes(): void {
    ...
    const handleSSE = async (req: express.Request, res: express.Response) => {
      ...
      console.log(`[GHL MCP HTTP] New SSE connection from: ${req.ip}, sessionId: ${sessionId}, tenant: ${tenantId}, method: ${req.method}`);
      ...
      try {
        ...
        console.log(`[GHL MCP HTTP] SSE connection established for session: ${sessionId}, tenant: ${tenantId}`);
        
        // Handle client disconnect
        req.on('close', () => {
          console.log(`[GHL MCP HTTP] SSE connection closed for session: ${sessionId}, tenant: ${tenantId}`);
        });
      ...
      res.json({
        ...
        endpoints: {
          ...
          admin: {
            ...
            updateTenant: '/admin/tenants/:tenantId (PUT)',
            deleteTenant: '/admin/tenants/:tenantId (DELETE)'
          }
        },
        tools: this.getToolsCount(),
  ...
  /**
   * Setup admin routes for tenant management
   */
  private setupAdminRoutes(): void {
    // Admin middleware
    const adminAuth = createTenantAdminMiddleware({
      adminApiKey: process.env.ADMIN_API_KEY
    });

    // List all tenants
    this.app.get('/admin/tenants', adminAuth, async (req, res) => {
      try {
        const tenants = await this.tenantConfigManager.getAllTenants();
    ...
    // Get specific tenant
    this.app.get('/admin/tenants/:tenantId', adminAuth, async (req, res) => {
      try {
        const tenant = await this.tenantConfigManager.getTenant(req.params.tenantId);
        if (!tenant) {
    ...
    // Create new tenant
    this.app.post('/admin/tenants', adminAuth, async (req, res) => {
      try {
        const tenant = await this.tenantConfigManager.createTenant(req.body);
    ...
    // Update tenant
    this.app.put('/admin/tenants/:tenantId', adminAuth, async (req, res) => {
      try {
        const tenant = await this.tenantConfigManager.updateTenant(
          req.params.tenantId,
          req.body
        );
        res.json(tenant);
    ...
    // Delete tenant
    this.app.delete('/admin/tenants/:tenantId', adminAuth, async (req, res) => {
      try {
        const deleted = await this.tenantConfigManager.deleteTenant(req.params.tenantId);
        if (!deleted) {
          return res.status(404).json({ error: 'Tenant not found' });
        }
        res.status(204).send();
    ...
    this.app.patch('/admin/tenants/:tenantId/status', adminAuth, async (req, res) => {
      try {
        const { isActive } = req.body;
        const tenant = await this.tenantConfigManager.setTenantActive(
          req.params.tenantId,
          isActive
        );
        res.json(tenant);
  ...
  async start(): Promise<void> {
    console.log('🚀 Starting GoHighLevel MCP HTTP Server (Multi-Tenant)...');
    console.log('===============================================');
    
    try {
      // Initialize default tenant if env vars are present
      if (process.env.GHL_API_KEY && process.env.GHL_LOCATION_ID) {
        console.log('🔧 Setting up default tenant from environment variables...');
        const defaultTenant = await this.tenantStore.get('default');
        if (defaultTenant) {
      ...
      this.app.listen(this.port, '0.0.0.0', () => {
        ...
        console.log('===============================================');
        console.log('📌 Tenant Identification Methods:');
        console.log('   - Header: X-Tenant-ID');
        console.log('   - Query: ?tenant=xxx');
        console.log('   - Default: Uses env vars if no tenant specified');
        console.log('===============================================');
      });
  ...
  /**
   * Get the Express app instance for external usage (e.g., Vercel)
   */
...
async function main(): Promise<void> {
  try {
    ...
    const server = new GHLMCPHttpMultiTenantServer();
    await server.start();
...
// Export the server class for external usage
export { GHLMCPHttpMultiTenantServer };
export default GHLMCPHttpMultiTenantServer;
